function [TrainNetworkOut,TestNetworkOut,ErrHistory] = BPGELUSig(TrainIn,TrainOut,TestIn,W1,B1,W2,B2,HiddenUnitNum,MaxEpochs,lr,E0,L,num_class,pL)
...
for i=1:MaxEpochs
    ...
    HiddenOut = 0.5*InToHidden.*(1+tanh(sqrt(2/pi)*(InToHidden+0.044715*InToHidden.^3)));
    ...
    Delta1=W2'*Delta2.*dGELU(InToHidden);
...
TrainHiddenOut = 0.5*TrainInToHidden.*(1+tanh(sqrt(2/pi)*(TrainInToHidden+0.044715*TrainInToHidden.^3)));
TrainNetworkOut=logsig(W2*TrainHiddenOut+repmat(B2,1,TrainNum));

%%%%%%%%%%%%%%%%%%%%%%%%% calculate the test error  %%%%%%%%%%%%%%%%%%%%%%%%%
TestInToHidden=W1*TestIn+repmat(B1,1,TestNum);
TestHiddenOut = 0.5*TestInToHidden.*(1+tanh(sqrt(2/pi)*(TestInToHidden+0.044715*TestInToHidden.^3)));
